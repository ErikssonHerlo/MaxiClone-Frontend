...
const CatalogStoreTable: React.FC<TableCustomProps> = ({
    ...
    useEffect(() => {
        const loadData = async () => {
            if (token && storeId) { // Solo ejecuta cuando storeId no es null o 0
                const result = await fetchData(`${endpoint}/${storeId}`, token);
                setData(result.data);
    ...
    useEffect(() => {
        ...
        if (userInfo && userInfo.storeId) {
            setStoreId(userInfo.storeId[0]); // Establece storeId después de obtener la información del usuario
        }
    }, []);
                        ...
                        >
                            ...
                            <img
                                ...
                                src={
                                    (book.product.coverImageUrl?.startsWith('http') ||
                                        ...
                                        !book.product.coverImageUrl?.startsWith('https://example.com')
                            ...
                            <div className="pl-2 py-2">
                                ...
                                <div className='flex justify-between items-center'>
                                    ...
                                    <h2 className={`text-sm ${book.storeStock > 0 ? 'text-green-500' : 'text-red-500'} px-4`}>{book.storeStock > 0 ? 'Unidades: ' + book.storeStock : 'No Disponible'}</h2>
                                </div>
